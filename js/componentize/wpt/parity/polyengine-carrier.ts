...
// module (`build/polyengine-carrier.mjs`) that both engine drivers load —
...
// polyengine's runtime linking: no transpile step, no generated tree, no
...
// conformance/driver-ct/polyengine/browser/worker-entry.ts's header, one
...
//
// THE POLYENGINE PIN IS SINGLE-SITE: this module resolves `@polyengine/*` through
// conformance/driver-ct/polyengine/deno.json (exact-pinned JSR releases;
...
// there also keeps `@polyengine/runtime/embedder` a single module instance
// across this bundle and js/polyengine/websocket.ts — one runtime/translator
...
import { websocketImports } from "../../../polyengine/websocket.ts";
import { reporter, setSink } from "../reporter.js";
import { socketsStubs } from "./sockets-stub-polyengine.mjs";
...
/**
 * Translate and instantiate the parity runner component under polyengine,
 ...
 * anywhere.
 *
 * @param translatorBytes the pinned polyengine translator shim
...
export async function instantiateRunner(
  ...
  const instance = await instantiate({ plan, componentBytes, adapters }, {
    ...
    // conformance leg uses (conformance/driver-ct/polyengine/run.ts).
    ...websocketImports(),
