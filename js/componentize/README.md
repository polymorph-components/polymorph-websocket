...
`websocket.js` is the inverse of `js/polyengine/websocket.ts`: where that module
...
**round trip** — shim → WIT → component ABI → polyengine →
`js/polyengine/websocket.ts` → the platform's own WebSocket — against the same suite echo server a
